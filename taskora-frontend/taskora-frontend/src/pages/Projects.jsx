...
const ProjectCard = ({ p }) => (
  <Link
    ...
    className="tk-card tk-job-card text-decoration-none text-dark"
  >
    <div
      className="tk-job-logo"
    ...
    <div className="flex-grow-1">
      ...
      <div className="d-flex flex-wrap gap-2">
        ...
        <span className={`tk-pill ${statusPill(p.projectStatus)}`}>
          {p.projectStatus}
        </span>
      </div>
    </div>
...
const Projects = () => {
  ...
  const load = (kw = "") => {
    setLoading(true);
    const fn = kw
      ? ProjectService.search({ keyword: kw })
      : ProjectService.getFeed();
    fn.then((r) => setProjects(r.data))
      .catch(() => setProjects([]))
    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="tk-card tk-empty">
          <i className="bi bi-folder2-open d-block mb-2"></i>No projects found.
        </div>
      ) : (
